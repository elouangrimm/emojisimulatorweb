...
(function(exports){ // Ensure EVERYTHING is inside this IIFE
    ...
    if(UI.options.bg==UI.NONE){
        ...
        if (css) css.innerHTML = "#grid_bg{ display:none; } #grid{color:#1c1b1f}"; // Use M3 text color
    }
    ...
    if (play_reset) {
        play_reset.onclick = function(){
            if (window.hasUnsavedChanges && !confirm("Resetting will discard unsaved changes. Continue?")) {
    ...
    const updatePauseUI = function(){ // Define before use
    ...
    if (playback_speed) {
        playback_speed.addEventListener('input', function(){ // Use addEventListener
    ...
    const _updateBrushIcon = function(){ // Define before use
        ...
        if (state) {
            ...
            play_draw.title = `Drawing: ${state.name} (${state.icon})`; // Update title
             ...
             if(Model.data.meta.draw !== 0){
                 console.warn(`Draw state ID ${Model.data.meta.draw} not found, resetting to 0.`);
    ...
    if (play_draw) {
        play_draw.onclick = function(){
            if (!Model.data || !Model.data.states || Model.data.states.length === 0) return;

            // Get current state and index
            const currentStateId = Model.data.meta.draw;
            const stateIndex = Model.data.states.findIndex(s => s.id == currentStateId); // Find by ID

            // Calculate next index, ensuring it loops correctly
            let nextIndex = 0; // Default to first state
            if (stateIndex !== -1) { // If current state was found
                 nextIndex = (stateIndex + 1) % Model.data.states.length;
            } else {
                 console.warn(`Current draw state ID ${currentStateId} not found. Resetting to first state.`);
            }
    ...
    subscribe("/ui/removeState", function(deleted_id){ // Update if current brush state is deleted
        if (Model.data && Model.data.meta && Model.data.meta.draw == deleted_id){ // Check ID type match if needed
    ...
    const getRealMouse = function(event){
        if (!Grid.dom || !Grid.tileSize) return; // Check if grid DOM exists
        const rect = Grid.domContainer.getBoundingClientRect(); // Use container rect
        Mouse.x = Math.floor((event.clientX - rect.left) / Grid.tileSize);
        Mouse.y = Math.floor((event.clientY - rect.top) / Grid.tileSize);
    };
    ...
    const getMousedTiles = function(){ // Calculate tiles covered by the line
    ...
    window.addEventListener("mouseup", function(event){
        if (event.button !== 0) return; // Only main button
        if (Mouse.pressed) { // If we were drawing
             Save.updateURL(); // Update URL when drawing stroke is finished
             Mouse.pressed = false;
        }
        if (scrubInput) { // If we were scrubbing
    ...
    if (Grid.domContainer) {
        Grid.domContainer.addEventListener("mouseleave", function(event){
            if (Mouse.pressed) {
