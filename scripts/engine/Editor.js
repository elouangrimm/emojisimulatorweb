...
(function(exports){
	...
	Editor.create = function(){
		...
		// Clear previous editor content if any (important for reloading)
		Editor.dom.innerHTML = '';
	
		///////////////////////
		///// TITLE INPUT ///// (Replaces Description - Request 5)
		///////////////////////
		...
		Editor.titleInput.value = Model.data.meta.title || "Untitled Emoji Simulation";
		Editor.titleInput.placeholder = "Simulation Title";
		Editor.titleInput.oninput = function() {
			const newTitle = Editor.titleInput.value || "Untitled Emoji Simulation";
			Model.data.meta.title = newTitle;
			document.title = newTitle + " - Emoji Simulator! 😘"; // Update tab title
			window.hasUnsavedChanges = true; // Mark changes
			// Debounced URL update might be good here too, or on blur/save
			 Save.updateURL();
		};
		Editor.titleInput.onchange = Save.updateURL; // Update URL when focus is lost too
		Editor.dom.appendChild(Editor.titleInput);
		// Keep reference for updates
		Editor.updateTitleUI = () => { // Helper to update UI from model
		...
		const addStateButton = Editor.createFancyButton(
			...
			function(){
				...
				Save.updateURL(); // Update URL on structural change
			}
		);
		...
		// Delete button
		const deleteButton = Editor.createFancyButton("<span class='button-icon'>⊗</span> Delete", function() {
			const selectedName = Editor.savedSimsDropdown.value;
			if (selectedName && selectedName !== "_placeholder_" && confirm(`Are you sure you want to delete the saved simulation "${selectedName}"?`)) {
				if(Save.deleteFromLocalStorage(selectedName)){
					 publish("/notify/info", [`Simulation "${selectedName}" deleted.`]);
				} else {
					 publish("/notify/error", [`Could not find simulation "${selectedName}" to delete.`]);
				}
			}
		});
		...
		// --- Share URL ---
		 ...
		 saveLoadSection.appendChild(Editor.createLabel("Get a shareable link for your current simulation:"));
		...
		fileInput.onchange = function(event) {
			...
			reader.onload = function(e) {
				try {
					...
					alert("Failed to import file. Please ensure it's a valid JSON simulation file.\n\n" + error.message);
		...
		// Export Button
		const exportButton = Editor.createFancyButton("<span class='button-icon'>📤</span> Export File", function(){
			try {
		...
		///// CREDITS /////
		//////////////////////
		if(UI.options.edit==UI.ADVANCED) { // Only show credits/links in advanced mode
			const creditsLabel = Editor.createLabel(`
				...
				<br>Modifications by AI Assistant.
			`);
	...
	Editor.populateSavedSimsDropdown = function() {
		...
		placeholderOption.disabled = true;
		 placeholderOption.selected = !saves.some(s => s.name === currentSelection); // Select if nothing else is
		...
		Editor.savedSimsDropdown.onchange = function() {
			...
			if (selectedName && selectedName !== "_placeholder_") {
				if (window.hasUnsavedChanges && !confirm("You have unsaved changes. Loading a simulation will discard them. Continue?")) {
				...
				if (selectedSave && selectedSave.data) {
					 try {
						...
						publish("/notify/success", [`Loaded "${selectedName}".`]);
					 } catch (e) {
						 console.error("Error parsing saved data:", e);
						 alert(`Error loading "${selectedName}". The saved data might be corrupted.`);
						  publish("/notify/error", [`Error loading "${selectedName}".`]);
					 }
				}
			}
		};
	...
	Editor.createStateUI = function(stateConfig){
		...
		dom.dataset.stateId = stateConfig.id; // Add data attribute for easier selection if needed
		...
		name.oninput = function(){
		 ...
		 name.onchange = Save.updateURL; // Update URL on blur too
		...
		// Actions
		const actionConfigs = stateConfig.actions = stateConfig.actions || []; // Ensure actions array exists
	...
	Editor.createActionsUI = function(actionConfigs, dom){
		...
		const addAction = Editor.createActionAdder(actionConfigs, dom);
		addEntry.appendChild(addAction);
	...
	Editor.createActionUI = function(actionConfig){
		if (!actionConfig || !actionConfig.type || !Actions[actionConfig.type]) {
	...
	Editor.createActionAdder = function(actionConfigs, domToUpdate){
		const keyValues = [
			{ name: "+ Add Action", value: "" } // Placeholder/Trigger text
		];
		...
		// Create a container for the button-like appearance
		const selectContainer = document.createElement("div");
		selectContainer.className ="editor_new_action";
		const buttonVisual = document.createElement("div"); // The visual part
		buttonVisual.innerHTML = "+ New Action";
		selectContainer.appendChild(buttonVisual);
		selectContainer.appendChild(select); // The actual select is invisible on top
	...
	Editor.createNumber = function(configObject, propName, options){
		...
		input.value = (configObject[propName] * options.multiplier).toFixed(options.integer ? 0 : 1);
		...
		input.oninput = function(){
			...
			const currentVal = options.integer ? parseInt(input.value) : parseFloat(input.value);
			 if (!isNaN(currentVal)) { // Only update model if it's a number
		...
		if (window._makeScrubbable) { // Check if function exists
	...
	Editor.createProportions = function(){
		...
		const populate = () => {
			...
			const proportions = Model.data.world.proportions = Model.data.world.proportions || [];
			...
			const newProportions = states.map(state => ({
			 ...
			 Array.prototype.push.apply(Model.data.world.proportions, newProportions); // Push new items
	
	
			// Calculate initial total for normalization
			let totalParts = newProportions.reduce((sum, p) => sum + p.parts, 0);
			if (totalParts <= 0) { // Avoid division by zero, set equal parts if all zero
			...
			newProportions.forEach((proportion, index) => {
				...
				slider.value = Math.round((proportion.parts / totalParts) * 100);
				...
				slider.oninput = function() {
					...
					if (otherTotal > 0) {
						...
						const remainingValue = (100 - currentValue) / (sliders.length - 1);
						sliders.forEach((s, i) => {
							if (i !== currentIndex) {
								 s.value = Math.round(remainingValue);
								 Model.data.world.proportions[i].parts = Math.round(remainingValue);
							 }
						});
						 // Update current slider's model data
						 Model.data.world.proportions[currentIndex].parts = currentValue;
					...
					sliders.forEach((s, i) => {
						if (i !== currentIndex) {
							...
							appliedTotal += scaledValue;
							 ...
							 Model.data.world.proportions[i].parts = currentValue; // Update model for current
						}
					});
					...
					if (diff !== 0 && sliders.length > 1) {
						 ...
						 if (Math.abs(diff) > 0 && sliders.length > 1) {
							 ...
							 sliders.forEach((s,i) => {
								 if (i !== currentIndex && parseFloat(s.value) > largestOtherVal) {
									 largestOtherVal = parseFloat(s.value);
									 largestOtherIdx = i;
								 }
							 });
							 adjustIndex = (largestOtherIdx !== -1 && parseFloat(sliders[adjustIndex].value) + diff < 0) ? largestOtherIdx : currentIndex;
							 if(parseFloat(sliders[adjustIndex].value) + diff >= 0){ // Ensure adjustment doesn't go below 0
								sliders[adjustIndex].value = parseFloat(sliders[adjustIndex].value) + diff;
								Model.data.world.proportions[adjustIndex].parts = parseFloat(sliders[adjustIndex].value);
							 } else {
		 ...
		 const normalizeProportions = () => {
			 ...
			 if (diff !== 0 && props.length > 0) {
				 ...
				 props.forEach((p, index) => {
				  ...
				  if(props[largestIndex].parts < 0) props[largestIndex].parts = 0; // Don't go below zero
				 if (sliders[largestIndex]) sliders[largestIndex].value = props[largestIndex].parts;
			 }
		 };
	...
	const EditorShortcuts = function(dom){
		...
		const _shortcut = function(funcName, EditorFunc){
			...
			self[nickname] = function(...args){ // Use rest parameters
	...
	exports.EditorHelper = function(tag = "span"){ // Default tag
